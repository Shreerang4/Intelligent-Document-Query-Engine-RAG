"""Smoke test: exercise retrieval modes and print ordered results.
...
from eval.pipeline_adapter import ingest_document, pipeline, query  # noqa: E402
...
def _print_results(label: str, chunks: List, latency_s: float) -> None:
    ...
    for rank, chunk in enumerate(chunks, start=1):
        print(f"  [{rank:>2}] page={chunk['page']}  chunk_id={chunk['chunk_id']}")
        print(f"        {_preview(chunk['text'])!r}")
...
def main() -> None:
    ...
    print(f"Embedder : {pipeline.get_embedding_model_name()}")
    print(f"Format   : {pipeline.get_embedding_input_format_version()}")
    if pipeline.uses_e5_embedding_format():
        print("Prefixes : documents use 'passage: '; queries use 'query: '")
    else:
        print("Prefixes : none")
